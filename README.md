...
3. **Results**
    ...
    - The script `3_from_intermediary_files_to_final_files.R` uses the intermediary datasets, either pre-computed and stored in `data/intermediary/` or generated using `2_from_raw_files_to_intermediary_files.R`, to compute the six proposed forecast combinations and the four benchmark methods. The resulting objects are stored in `data/results/`.
    - The script `4_replicate_figure_2_and_tables_5_to_7.R` reproduces Figure 2 and Tables 5--7 directly using the pre-computed files included in `data/results/` or those generated through the entire workflow.
